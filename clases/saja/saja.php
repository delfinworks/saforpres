...
class Saja {
	...
	function saja_status($style='', $string='Procesando...')
	{
		return "<div id=\"sajaStatus\" style=\"visibility:hidden;$style\"  width='1024' height='768' >".htmlentities($string)."</div>";
	}
...
/*if(!(eregi('iniciar_sesion',$function) || eregi('chequea_cedula',$function) || eregi('guardar_password_inicio',$function) || eregi('restablecer_password',$function)))
//if(!('iniciar_sesion'==$function) || ('chequea_cedula'==$function) || ('guardar_password_inicio'==$function) || ('restablecer_password'==$function)))
		{
			include_once('../../modulos/login/ObjetoLogin.class.php');
			$login_usr = new login_usr();
			$res = $login_usr->Chequear_sesion();
			if(!$res)
			{		
				$sesion = "0"; //envío al usuario a la pag. de autenticación
			...
			if ($sesion == "0" && $_SESSION['rol']!= "admin" )
			{
				return "window.top.location.href='../../index.php?cerrar=cerrar';";
			}
			else
			{*/
				return $all_commands;
